    ...
    <section id="hero" className="min-h-screen relative flex items-center justify-center overflow-hidden bg-gradient-to-br from-slate-950 via-gray-900 to-black">
      ...
      <div className="relative z-10 text-center px-6 max-w-7xl mx-auto">
        ...
        <div className="mb-8 animate-fade-in animation-delay-300">
          <h1 className="text-8xl md:text-9xl lg:text-[12rem] font-black mb-4 leading-none animate-text-glow animate-glow">
            <span className="block text-white drop-shadow-[0_0_40px_rgba(255,255,255,0.8)] animate-glow animate-text-glow">
              Aswin
            </span>
          </h1>
        ...
        {/* Location Badge */}
        <div className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-orange-500/40 via-yellow-500/40 to-emerald-500/40 rounded-full border-2 border-orange-500/50 mb-12 animate-fade-in animation-delay-600 hover:shadow-2xl hover:shadow-orange-500/60 transition-all duration-500 animate-pulse-glow-enhanced group shadow-2xl shadow-orange-500/30 animate-glow animate-gradient animate-neon-glow">
          <span className="w-8 h-6 flex items-center justify-center rounded-sm overflow-hidden border border-orange-500/60 bg-white">
            {/* Clear SVG India flag */}
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 20" width="32" height="20">
              <rect width="28" height="20" fill="#fff"/>
              <rect width="28" height="6.67" y="0" fill="#ff9933"/>
              <rect width="28" height="6.67" y="13.33" fill="#138808"/>
              <circle cx="14" cy="10" r="2.2" fill="none" stroke="#000088" strokeWidth="0.8"/>
              <g stroke="#000088" strokeWidth="0.3">
                {[...Array(24)].map((_,i) => (
                  <line key={i} x1="14" y1="10" x2={14+2.2*Math.cos((i*Math.PI)/12)} y2={10+2.2*Math.sin((i*Math.PI)/12)}/>
                ))}
              </g>
            </svg>
          </span>
          <span className="text-slate-200 font-bold group-hover:text-white transition-colors animate-neon-glow-enhanced animate-text-glow animate-glow">Based in India</span>
        </div>
